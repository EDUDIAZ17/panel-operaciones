...
export async function renderATCReports(container) {
    ...
    document.getElementById('atc-report-type').addEventListener('change', loadATCReport);
    document.getElementById('atc-filter-status').addEventListener('change', loadATCReport);
    document.getElementById('atc-filter-unidad').addEventListener('keyup', (e) => {
        if(e.key === 'Enter') loadATCReport();
    });
    document.getElementById('btn-refresh-atc-report').addEventListener('click', loadATCReport);
    document.getElementById('btn-export-atc-report').addEventListener('click', exportATCReport);

    loadATCReport();
}

async function loadATCReport() {
    const type = document.getElementById('atc-report-type').value;
    const filterTxt = document.getElementById('atc-filter-unidad').value.toLowerCase();
    const filterStatus = document.getElementById('atc-filter-status').value;
    ...
    if (error) {
        tbody.innerHTML = `<tr><td colspan="17" class="p-8 text-center text-red-500 font-bold">Error: ${error.message}</td></tr>`;
        return;
    }
    ...
    if (filteredUnits.length === 0) {
        ...
        filteredUnits.forEach((u, idx) => {
            ...
            if (samsaraVeh) {
                ...
                setTimeout(() => reverseGeocodeStatic(samsaraVeh.location.latitude, samsaraVeh.location.longitude, geoId), idx * 100);
            } else {
